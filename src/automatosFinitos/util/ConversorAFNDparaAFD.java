...
public class ConversorAFNDparaAFD {
	...
	public static AFD converter(AFND automato) throws EntradaIndefinidaException{
		...
		// O set de estados que � definido como inciial � aquele obtido do estado inciial do AFND com a entrada nula (EPSILON)
		sets.add(setInicial);
	...
	/**
	 * Retorna um ArrayList contendo todos os estados alcan��veis a partir do estado dado e lida a entrada dada,
	 * que pode tamb�m ser simplemente EPSILON.
	 * 
	 * @param inicial Estado AFND do qual se parte para  a an�lise.
	...
	private static void addAlcancaveis(ArrayList<EstadoAFND> registrados, EstadoAFND atual, String entrada){
		...
		for(int i=0; i < atual.getChaves().size(); i++){ //transicoes EPSILON que podem ser feitas sem ler entrada
			if(chaves.get(i).equals(EstadoAFND.EPSILON))
				addAlcancaveis(registrados, estados.get(i), entrada); //chamada recursiva para cada estado alcan�ado por EPSILON
		}
		
		if (entrada != EstadoAFND.EPSILON){ //checa se h� uma entrada para ser lida	
			for(int i=0; i < atual.getChaves().size(); i++){ //transicoes compativeis com a entrada
				if(chaves.get(i).equals(entrada))
					addAlcancaveis(registrados, estados.get(i), EstadoAFND.EPSILON); //chamada recursiva para cada estado alcan�ado ao ler a entrada, agora sem poder l�-la novamente
			}
		}
	}
	...
	private static void addSetsResultantes(ArrayList<SetDeConversao> conhecidos, SetDeConversao atual){
		...
		for(int i=0; i < alfabeto.length; i++){ //para cada chave do alfabeto de entradas gera um set resultante
			...
			atual.setTransicao(alfabeto[i], resultante); //define a transi��o do atual para o resultante de acordo com a entrada que o gerou.
	...
	private static AFD gerarAFD(ArrayList<SetDeConversao> sets) throws EntradaIndefinidaException{
		...
		for(int i=0; i < sets.size(); i++){
			if(sets.get(i).isFinal()) estados.get(i).setFinal(true); //para cada estado equivalente a um set que cont�m um final, o estado � final
			for(int j=0; j < alfabeto.length; j++){
				int indResultante = getIndiceSet(sets, sets.get(i).getSetTransicao(alfabeto[j])); //O indice equivalente do set resultante da transi��o 
				estados.get(i).setFuncaoTransicao(alfabeto[j], estados.get(indResultante)); //O estado de mesmo indice � resultante dessa mesma transi��o no estado equivalente iterado
			}
		}
		
		AFD automato = new AFD(alfabeto);
		EstadoAFD inicial = estados.get(0); //o primeiro set � o set que cont�m o estado inicial sem ler entrada, logo o primeiro estado � o inicial.
