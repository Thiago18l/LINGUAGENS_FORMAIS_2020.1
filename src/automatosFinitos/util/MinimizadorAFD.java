...
/**
 * Converte um AFD dado para sua forma minima (equivalente com menor numero de estados poss�veis par).
 */
...
public class MinimizadorAFD {
	...
	private static AFD gerarAFD(LinkedList<Particao> particoes, String[] alfabeto) throws EntradaIndefinidaException{
		...
		estados.get(estados.size()-1).setFinal(true); //sendo a particao que cont�m os estados finais a ultima adionada, o estado equivalente � final
		...
		for(int i=0; i < particoes.size(); i++){
			for(int j=0; j < alfabeto.length; j++){
				...
				//O estado que est� na particao resultante � resultante para a mesma entrada no estado equivalente (mesmo �ndice)
			}
		}
